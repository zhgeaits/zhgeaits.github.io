...
由2.1知道稀疏矩阵改用一个3列的矩阵存储，前面两列存储的是元素在原矩阵的坐标，第三列存储的是元素的值。其实这三列就是三个数组，第几行则是数组的下标索引。而存储map的kv对的时候，因为并不是矩阵，所以无需用到三列的矩阵存储，两列即可，一列存储key，另外一列存储value，即我们的int数组和object数组。